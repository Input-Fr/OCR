...
void initNetwork(Network* network, int* sizes) {
    ...
    for (int i = 0; i < network->sizes[1] * network->sizes[2]; i++) {
        ...
        while (i < (network->sizes[1] * network->sizes[2]) && fgets(buffer, sizeof(buffer), filew2) != NULL) {
                ...
                printf("w2 : %s -> %f\n",buffer,network->weightsHiddenOutput[i]);
		i++;
...
char maxOutput(double* output)
{
	...
	for(int i = 0; i<10;i++)
	{
		if (1 != output[i] && output[i]>output[max])//(output[i] > 0.7 && output[i]>output[max])
		{
			max = i;
		}
	}
	if ((output[6] == 0 || output[6] == 1) && output[max] < 0.90)
	
	{
		return '6';
	}
	if ((output[5] == 0 || output[5] == 1) && output[max] < 0.9)
	{
		return '5';
	}
	if (max == 0)
